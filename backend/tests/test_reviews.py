...
async def test_create_review_returns_201(client, auth_headers):
    ...
    r = await client.post('/api/reviews', json={'product_id': pid, 'rating': 5, 'comment': 'Excellent.'}, headers=auth_headers)
...
async def test_duplicate_review_returns_409(client, auth_headers):
    pid = await _pick_product_id(client)
    first = await client.post('/api/reviews', json={'product_id': pid, 'rating': 4, 'comment': 'Good.'}, headers=auth_headers)
    assert first.status_code == 201, first.text
    second = await client.post('/api/reviews', json={'product_id': pid, 'rating': 3, 'comment': 'Changed my mind.'}, headers=auth_headers)
    assert second.status_code == 409

@pytest.mark.asyncio
async def test_review_for_unknown_product_returns_404(client, auth_headers):
    r = await client.post('/api/reviews', json={'product_id': str(uuid.uuid4()), 'rating': 5}, headers=auth_headers)
    assert r.status_code == 404

@pytest.mark.asyncio
async def test_invalid_rating_returns_422(client, auth_headers):
    pid = await _pick_product_id(client)
    too_high = await client.post('/api/reviews', json={'product_id': pid, 'rating': 6}, headers=auth_headers)
    assert too_high.status_code == 422
    too_low = await client.post('/api/reviews', json={'product_id': pid, 'rating': 0}, headers=auth_headers)
