"""Tests for the flash sale flow.

Covers:
  - Admin-only creation (403 for customers)
  - Active listing
  - Atomic claim decrements stock
  - Sold-out path returns 409 and does not over-allocate
  - Claim on a future-window sale returns 409
"""
...
def _sale_payload(product_id: str, *, stock: int = 2, starts_in: int = -60, lasts: int = 3600):
    """Build a FlashSaleCreate payload. `starts_in` and `lasts` are seconds."""
    now = datetime.now(timezone.utc)
    return {
        "product_id": product_id,
        "sale_price": "9.99",
        "original_price": "19.99",
        "start_at": (now + timedelta(seconds=starts_in)).isoformat(),
        "end_at": (now + timedelta(seconds=starts_in + lasts)).isoformat(),
        "initial_stock": stock,
    }


async def _pick_product_id(client) -> str:
    """Grab any product id from the seeded catalog."""
...
async def test_claim_decrements_stock_and_creates_order(client, admin_headers, auth_headers):
    ...
    sale = (await client.post(
        "/api/flash-sales", json=_sale_payload(pid, stock=2), headers=admin_headers
    )).json()

    r = await client.post(
        f"/api/flash-sales/{sale['id']}/claim",
        json={"shipping_address": "1 Test Street"},
...
async def test_claim_returns_409_when_sold_out(client, admin_headers, auth_headers, registered_user):
    """Drain a 1-stock sale, then a second customer hitting it gets 409 sold_out."""
    pid = await _pick_product_id(client)
    sale = (await client.post(
        "/api/flash-sales", json=_sale_payload(pid, stock=1), headers=admin_headers
    )).json()

    # First customer drains the only unit
    ...
    # Second customer (a fresh registration) tries -- should fail
    import uuid
    email = f"buyer+{uuid.uuid4().hex[:8]}@example.com"
    reg = await client.post(
        "/api/auth/register",
        json={"email": email, "password": "password123", "name": "Buyer"},
    )
    other = {"Authorization": f"Bearer {reg.json()['tokens']['access_token']}"}

    r2 = await client.post(
        f"/api/flash-sales/{sale['id']}/claim",
        json={"shipping_address": "2 Other Street"},
        headers=other,
    )
...
async def test_claim_returns_409_when_sale_not_running(client, admin_headers, auth_headers):
    """A sale starting in the future returns 409, not 200, on claim."""
    pid = await _pick_product_id(client)
    future = (await client.post(
        "/api/flash-sales",
        json=_sale_payload(pid, starts_in=3600, lasts=3600),
        headers=admin_headers,
    )).json()

    r = await client.post(
        f"/api/flash-sales/{future['id']}/claim",
        json={"shipping_address": "1 Test Street"},
        headers=auth_headers,
    )
