"""Tests for the token-bucket rate limiter (R11).

Covers:
  - Requests within capacity pass through, X-RateLimit-* headers are correct.
  - When tokens are exhausted a deny + Retry-After is returned.
  - After waiting (or manual refill) requests pass again.
  - Distributed variant on fakeredis: the atomic Lua script gives the same contract.
  - Client isolation: a limit on client_id A does not affect B.
"""
import asyncio
...
async def test_in_memory_refills_after_wait():
    rl = InMemoryRateLimiter(max_tokens=1, refill_rate=10.0)  # 100ms per token
