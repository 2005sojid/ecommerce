...
async def get_cart(user: CustomerUser, db: DBSession) -> CartOut:
    return await _service.get_cart(user.id, db)

@router.post('/items', response_model=CartOut, status_code=status.HTTP_201_CREATED)
async def add_item(payload: CartItemAdd, user: CustomerUser, db: DBSession) -> CartOut:
...
async def clear_cart(user: CustomerUser) -> None:
