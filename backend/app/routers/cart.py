...
async def get_cart(user: CurrentUser, db: DBSession) -> CartOut:
    """Current user's cart (reads from Redis, enriches with product data)."""
...
async def add_item(payload: CartItemAdd, user: CurrentUser, db: DBSession) -> CartOut:
    """Add a product to the cart. Checks existence and available stock."""
    product = await db.get(Product, payload.product_id)
    if product is None or not product.is_active:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Product not found")
    inv = (
        await db.execute(select(Inventory).where(Inventory.product_id == payload.product_id))
    ).scalar_one_or_none()
...
async def update_item(
    product_id: uuid.UUID, payload: CartItemUpdate, user: CurrentUser, db: DBSession
) -> CartOut:
    """Change quantity. If quantity=0 -- the item is removed from the cart."""
