...
async def add_to_wishlist(payload: WishlistAdd, user: CurrentUser, db: DBSession) -> dict:
    await wishlist_service.add(db, user.id, payload.product_id)
    return {'ok': True, 'product_id': str(payload.product_id)}


@router.delete('/{product_id}', status_code=status.HTTP_204_NO_CONTENT)
async def remove_from_wishlist(product_id: uuid.UUID, user: CurrentUser, db: DBSession) -> Response:
    await wishlist_service.remove(db, user.id, product_id)
    return Response(status_code=status.HTTP_204_NO_CONTENT)


@router.get('')
async def list_wishlist(user: CurrentUser, db: DBSession, page: int = Query(1, ge=1), per_page: int = Query(20, ge=1, le=100)) -> dict:
...
@router.get('/ids')
async def list_wishlist_ids(user: CurrentUser, db: DBSession) -> list[uuid.UUID]:
