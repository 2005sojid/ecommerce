...
async def create_flash_sale(
    payload: FlashSaleCreate, _: AdminUser, db: DBSession
) -> FlashSaleOut:
    """Create a flash sale. Stock is preloaded into Redis (`flash:{id}:stock`)."""
    ...
    sale = FlashSale(
        ...
        remaining_stock=payload.initial_stock,
        is_active=True,
    )
    db.add(sale)
    await db.commit()
...
async def active_flash_sales(db: DBSession) -> list[FlashSaleOut]:
    """List of currently active sales."""
...
async def claim(
    sale_id: uuid.UUID, payload: CheckoutRequest, user: CurrentUser, db: DBSession
) -> FlashSaleClaimResponse:
    """Buy a product at the flash price. Atomic DECRBY in Redis. Rate limiter -- in Phase 6."""
