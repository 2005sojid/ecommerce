...
async def _search_via_meili(
    ...
    res = await search_service.search(
        ...
        page=page,
        per_page=per_page,
        sort_by=sort_by if sort_by in ("price", "created_at") else None,
    ...
    return Page[ProductOut](
        items=[ProductOut.model_validate(p) for p in ordered],
...
async def list_products(
    ...
    sort_by: Annotated[str, Query(pattern="^(price|created_at|name)$")] = "created_at",
    sort_order: Annotated[str, Query(pattern="^(asc|desc)$")] = "desc",
) -> Page[ProductOut]:
    """Product list with pagination, filters, and sorting.

    If `search` is given -- executed via Meilisearch (typo-tolerant, faceted, ranked).
    Otherwise -- a plain SQL query.
    """
    if search:
        try:
            return await _search_via_meili(
                db, search, page, per_page, category_id, min_price, max_price, sort_by, sort_order
            )
...
async def get_product(product_id: uuid.UUID, db: DBSession) -> ProductDetail:
    """Product detail card. Cache-aside: key `product:{id}`, TTL 5 min.

    Rating is cached separately (`product:{id}:avg_rating`, TTL 10 min) --
    invalidated only when a new review is added.
    """
...
async def create_product(payload: ProductCreate, _: AdminUser, db: DBSession) -> ProductOut:
    """Creates a product and the related inventory row (admin only).

    Indexing into Meilisearch is async via RabbitMQ
    (event `product.created` -> worker `search_sync`).
    """
    product = Product(
        id=uuid.uuid4(),
        name=payload.name,
...
async def update_product(
    product_id: uuid.UUID, payload: ProductUpdate, _: AdminUser, db: DBSession
) -> ProductOut:
    """Updates a product (admin only). Indexing happens via RabbitMQ -> search_sync."""
    product = await db.get(Product, product_id)
    if product is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Product not found")
...
async def delete_product(product_id: uuid.UUID, _: AdminUser, db: DBSession) -> None:
    """Soft-delete: sets is_active=False (admin only). Index removal happens via the worker."""
    product = await db.get(Product, product_id)
    if product is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Product not found")
...
async def list_product_reviews(
    ...
    """List of reviews for a product (paginated)."""
