...
def _build_tree(categories: list[Category]) -> list[CategoryNode]:
    by_id: dict[uuid.UUID, CategoryNode] = {
        c.id: CategoryNode(id=c.id, name=c.name, slug=c.slug, parent_id=c.parent_id)
        for c in categories
    }
...
async def list_categories(db: DBSession) -> list[CategoryNode]:
    """Category tree. Cache-aside: key `categories:tree`, TTL 30 min."""
...
async def category_products(
    ...
    """List of products in a category (paginated)."""
