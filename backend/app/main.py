...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Workers subscribe to their queues (consume -- non-blocking, runs in the background).
...
app = FastAPI(
    ...
    description="Real-time inventory e-commerce system with flash sales, order tracking, and analytics.",
    version="1.0.0",
    docs_url="/api/docs",
    redoc_url="/api/redoc",
...
# Observability: traces, metrics (structured logging is already configured in configure_logging).
try:
...
# Rate limit: flash-sale claim and login. 30 tokens, refill 0.5/sec (60s full refill).
app.add_middleware(
    RateLimitMiddleware,
    limiter=RedisRateLimiter(redis_client, max_tokens=30, refill_rate=0.5),
    paths=["/api/flash-sales/", "/api/auth/login"],
)
