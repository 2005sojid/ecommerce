"""Event-driven order fulfilment pipeline (consumer queue `order_pipeline`).

Stages:
  order.created    -> verify payment (mock 2s) -> status=confirmed   -> publish order.confirmed
  order.confirmed  -> reserve confirm          -> status=processing  -> publish order.processing
  order.processing -> packing (mock 3s)        -> status=packed      -> publish order.packed
  order.packed     -> handoff to carrier       -> status=shipped     -> publish order.shipped

Each step:
  - reads an event from RabbitMQ
  - updates orders.status in Postgres
  - inserts a row into order_events (audit trail)
    ...
    async with async_session() as db:
        ...
        db.add(
            OrderEvent(
                ...
                event_metadata={"source": "pipeline"},
            )
        )
        await db.commit()
    logger.info("order %s: %s -> %s", order_id, old_status.value if old_status else None, new_status.value)
    await broadcast_order_status(
        order_id, new_status.value, old_status.value if old_status else None
    )
...
async def run() -> None:
    """Runs as asyncio.create_task() from main.lifespan."""
