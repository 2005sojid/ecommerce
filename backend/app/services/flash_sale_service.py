...
class FlashSaleService:
    ...
    async def claim(
        self, sale_id: uuid.UUID, user_id: uuid.UUID, shipping_address: str, db: AsyncSession
    ) -> tuple[Order, int]:
        ...
        db.add(
            OrderEvent(
                ...
                event_metadata={"source": "flash_sale", "sale_id": str(sale_id)},
            )
        )
        ...
        await publish_event(
            "order.created",
            {"order_id": order.id, "source": "flash_sale", "sale_id": str(sale_id)},
        )
