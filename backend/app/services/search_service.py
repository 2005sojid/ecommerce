...
- In Phase 3 -- synchronous call from the product router (CRUD).
- In Phase 5 this will be replaced by a RabbitMQ consumer (`product.*` events) --
  direct router calls become publish_event, and the worker `search_sync.py`
  becomes the consumer that does the indexing (eventual consistency by design).
"""
import logging
import uuid
...
class SearchService:
    ...
    async def init_index(self) -> None:
        """Creates the index (if missing) and configures attributes. Call on startup."""
        try:
    ...
    async def search(
        ...
        return {
            "hits": result.hits,
            "estimated_total_hits": result.estimated_total_hits or 0,
        }

    async def reindex_all(self, db: AsyncSession) -> int:
        """Full reindex (for bootstrap / restore)."""
        rows = (
