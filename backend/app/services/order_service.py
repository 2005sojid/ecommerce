...
class OrderService:
    ...
    async def checkout(
        self, user_id: uuid.UUID, shipping_address: str, db: AsyncSession
    ) -> Order:
        ...
        for product_id_str, quantity in cart_items.items():
            ...
            if product is None or not product.is_active:
                raise HTTPException(
                    status.HTTP_404_NOT_FOUND, f"Product {product_id} unavailable"
                )
    ...
    async def update_status(
        ...
        await publish_event(
            f"order.{new_status.value}",
            {"order_id": order.id, "status": new_status.value},
        )
        await broadcast_order_status(
            order.id, new_status.value, old_status.value if old_status else None
        )
