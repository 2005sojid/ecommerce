"""Cache-aside helpers on top of Redis.

Used in routers directly (cache_get / cache_set / cache_delete).
Also exports the `cached` decorator -- for functions with a deterministic key.
"""
import json
import logging
...
def _key_pattern(key: str) -> str:
    """Group keys by prefix for Prometheus labels (low cardinality)."""
...
async def cache_get(key: str) -> Any | None:
    try:
        ...
        return None
    # Lazy import -- avoid circular (metrics -> prometheus_client tcp).
...
def cached(key_template: str, ttl: int = 300) -> Callable:
    """Cache-aside decorator for async functions.

    `key_template` is formatted via the call's **kwargs, for example:
